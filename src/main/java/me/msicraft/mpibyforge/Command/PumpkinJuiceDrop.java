...
public class PumpkinJuiceDrop {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        ...
        dispatcher.register(Commands.literal("mpi").requires(cs -> cs.hasPermissionLevel(4))
                .then(Commands.literal("pumpkinjuicedroprate").executes(command -> getPumpkinJuiceDropRate(command.getSource()))));
        dispatcher.register(Commands.literal("mpi").requires(cs -> cs.hasPermissionLevel(4))
        ...
        dispatcher.register(Commands.literal("mpi").requires(cs -> cs.hasPermissionLevel(4))
                .then(Commands.literal("pumpkinjuicedroplevelrange").executes(command -> getPumpkinJuiceDropLevelRange(command.getSource()))));
    }
    ...
    private static int getPumpkinJuiceDropRate(CommandSource commandSource) {
        if (commandSource.getEntity() instanceof PlayerEntity) {
            PlayerEntity player = (PlayerEntity) commandSource.getEntity();
            player.sendMessage(new StringTextComponent("현재 pumpkinjuicedroprate 값: " + EntityRelated.getPumpkinJuiceDropRate()));
        } else {
            commandSource.sendFeedback(new StringTextComponent("현재 pumpkinjuicedroprate 값: " + EntityRelated.getPumpkinJuiceDropRate()), false);
        }
        return 1;
    }
    ...
    private static int getPumpkinJuiceDropLevelRange(CommandSource commandSource) {
        if (commandSource.getEntity() instanceof PlayerEntity) {
            PlayerEntity player = (PlayerEntity) commandSource.getEntity();
            player.sendMessage(new StringTextComponent("현재 pumpkinjuicedroplevelrange 값: " + EntityRelated.getPumpkinJuiceDropLevelRange()));
        } else {
            commandSource.sendFeedback(new StringTextComponent("현재 pumpkinjuicedroplevelrange 값: " + EntityRelated.getPumpkinJuiceDropLevelRange()), false);
        }
        return 1;
    }
